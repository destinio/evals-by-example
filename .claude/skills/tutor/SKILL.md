...
grep -ln "wrapOpenAI\|initLogger" app/*.ts 2>/dev/null || echo "app not instrumented yet"
ls learn/ && ls app/*.ts
```
...
**Their work stays on their branch.** Everything happens in their clone on `my-course`. Commit whenever it's useful. Don't suggest worktrees or per-step branches — keep the setup to what someone who just cloned the repo can follow.

## The course

Nine steps. Only some have a written file in `learn/` yet — when the one they're on doesn't, teach it from the outline below; the plan is complete even where the write-up isn't.
...
- **Step 2+:** run their eval script, read the actual scores, and look for the mistakes below before believing any number.
...
- Learners work on the `my-course` branch; `main` stays free of Braintrust (instrumenting it is step 1). Starting over: `git switch main` and delete `app/data/happytails.db`.
